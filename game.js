...
var gameTicTacToe = (function() {
  ...
  var computerAI = (function() {
    ...
    var checkCorner = function () {                                             //to check if corners are occupied by computer or human and if human occupied a pair of diagonal corner
      var xOccupy = false;
      var oOccupy = false;
      var oDiagnoal = false;
      if ((squareMatrix[0] == 1) || (squareMatrix[2] == 1) || (squareMatrix[6] == 1) || (squareMatrix[8] == 1) ) {
        xOccupy = true;
      }
      if ((squareMatrix[0] == -1) || (squareMatrix[2] == -1) || (squareMatrix[6] == -1) || (squareMatrix[8] == -1) ) {
        oOccupy = true;
      ...
      if (oOccupy) {
        if(((squareMatrix[0] == -1) && (squareMatrix[8] == -1)) || ((squareMatrix[2] == -1) && (squareMatrix[6] == -1))) {
          oDiagnoal = true;
        }
      }
    ...
    var occupyCorner = function () {                                            //occupy any corner
      for (var i = 0; i < 9; i++) {
    ...
    var occupyDiagnoalCorner = function() {                                     // //occupy a diagonal corner
      for (var i = 0; i < 9; i++) {
        if (squareMatrix[i] == 0 && (i == 0 || i == 2 || i == 6 || i == 8)) {     //if  one corner is availble, check if it's diagonalCorner is occupied by computer
    ...
    var occupySquare = function() {                                             //occupy any square
      for (var i = 0; i < 9; i++) {
        if (squareMatrix[i] == 0) {
          return (i + 1);
        }
    ...
    var occupySameSideCorner = function() {                                     //if computer has occupied one corner, and the other 2 squares on the same line are available,
      var sameSide1, sameSide2, middle1, middle2;                               //occupy the corner on the other end of this line.
      for (var i = 0; i < 9; i++) {
        if ((squareMatrix[i] == 1) && ((i == 0) || (i == 2) || (i == 6) || (i == 8))) {    //if one corner is occupied by computer
          if (((i + 2) > 8) || ((i + 2) == 4)) {                                           //find the two corners which are on the other end of lines.
            sameSide1 = i - 2;
          }
          else {
            sameSide1 = i + 2;
    ...
    var checkComputerWin = function() {                                         //to check if computer will win in the next step.
      var winFlag;
      for (var i = 0; i < 9; i++ ) {                                             //first step, check if computer can win if one square is clicked
        if (squareMatrix[i] == 0) {                                              //the square is availble
    ...
    var checkHumanWin = function() {                                            //to check if human will win in the next step
      var WinFlag;
      for (var i = 0; i < 9; i++ ) {
        if (squareMatrix[i] == 0) {
          squareMatrix[i] = -1;
    ...
    var tryOccupyCorner = function () {                                         //this function defines the strategy how the computer occupies the corner.
   ...
   var computerThink = function() {                                              //computer makes a decision from this function
      ...
      if (computerWinFlag) {                                                      //if yes, return this value to ensure computer wins in next step
        return computerWinFlag;
      }

      var humanWinFlag = checkHumanWin();                                        //2. check wether human will win if human clicks this square
      if (humanWinFlag) {                                                             //if yes, return this value to prevent computer from winning in next step
        return humanWinFlag;
      }
                                                                                //3 if no one can win
      if (xCornerOccupy) {                                                       //check if computer has occupied at least one corner

        if(oCornerOccupy) {                                                     //if human has occupied one corner
        ...
        if (centerOccupy == 0) {                                                // check if center square is available.
    ...
    else {                                                                      //if computer does not occupy any corner, occupy one corner
          if (oCornerOccupy && !computerFirst) {                                //check if human has occupied at least one corner and is the first player
            if (!centerOccupy) {                                                //if yes, occupy the center
              return 5;
            }
            if (oDiagnoalOccupt) {                                              //if human has occupied a pair of diagnoal corners, occupied square 4 or square 6
  ...
  var gameModeSelect = function() {                                             //select a gmae mode, 1 player or 2 players
  ...
  var getUserInfo = function() {
    ...
    if (xUserName == oUserName) {
      do {
        oUserName = prompt("O's player name cannot be the same as X's player's name, please enter O's player name");
 ...
 var generateGameBoard = function() {
    ...
    for (var i = 1; i < 10; i++) {                                              //to create 9 squares
  ...
  var initGame = function() {
    ...
    if (firstTime) {                                                            //get users information if it is the first time
      getUserInfo();
  ...
  var nextRound = function() {
   ...
   var currentName;
   if ( isFirstRound ) {                                                        //if it is the first round, choose the first player randomly
   ...
   if (xUserTurn) {                                                             //define sign, color of sign, "X" user name in "X" user's turn
      sign = "X";
      signColor = "chartreuse";
      currentName = xUser.name;

   }
   else {                                                                       //define sign, color of sign, "O" user name in "O" user's turn
      sign = "O";
      signColor = "red";
  ...
  var computerMode = function() {                                               //computer will act as 'X' user in this game.
    if (onePlayerMode) {
      if (xUserTurn) {
          computerAction();
          checkGameState();
  ...
  var computerAction = function() {                                             //the action the computer plays
  ...
  var firstGo = function() {                                                    //if computer first, this function is to decide which square computer will click
  ...
  var selectSquare = function(squareId, recordNumber) {
    ...
    if (squareMatrix[id] != 0) {                                                //if square is not empty, alert an error message and return false
      alert("This position is not availble, please select another position")
      return false;
    }
    else {                                                                      //if square is empty, put the record to square matrix
  ...
  var checkWin = function() {
    ...
    else if (Math.abs(squareMatrix[3] + squareMatrix[4] + squareMatrix[5]) == 3) {

        return [true, "metrix4"];
    }
    else if (Math.abs(squareMatrix[6] + squareMatrix[7] + squareMatrix[8]) ==3) {

        return [true, "metrix5"];
    }
    else if (Math.abs(squareMatrix[0] + squareMatrix[3] + squareMatrix[6]) ==3) {

        return [true, "metrix6"];
    }
    else if (Math.abs(squareMatrix[1] + squareMatrix[4] + squareMatrix[7]) ==3) {

        return [true, "metrix7"];
    }
    else if (Math.abs(squareMatrix[2] + squareMatrix[5] + squareMatrix[8]) ==3) {

        return [true, "metrix8"];
    }
    else if (Math.abs(squareMatrix[0] + squareMatrix[4] + squareMatrix[8]) ==3) {

      return [true, "metrix1"];

    }
    else if (Math.abs(squareMatrix[2] + squareMatrix[4] + squareMatrix[6]) ==3) {
   ...
   var checkGameState = function() {
      ...
      if (win) {                                                                //if someone wins, draw a  line to connect the 3 squares.
        drawingLine(way);
        ...
        if (xUserTurn) {
         ...
         if (step < 8) {                                                        //if no one wins and it is not the last round, continue next round
            step += 1;
            xUserTurn = !xUserTurn;
            nextRound();
         }
         else {                                                                 //if it is the last round and no one wins, display a tie
   ...
   var drawingLine = function(showList) {                                       //to draw a line to connect the 3 squares
   ...
   var promptWinner = function(winnerName, result) {                            //to prompt a window to show the result of this match.
     ...
     if (result) {                                                               //if wins, show who wins
       winMessage.innerHTML = winnerName + " wins";
     }
     else {                                                                      //if no one wins, show a tie
 ...
 var goBack = function() {                                                      //to close result display
